document.addEventListener('DOMContentLoaded', (event) => {
    ...
    let figureNameList = ['line - линия', 'rectangle - прямоугольник', 'triangle - треугольник', 'circle - круг', 'ellipse - овал'];
    for (let figureName of figureNameList) {
    ...
    let optionList = ['-p [x, y] координаты',
        '-r радиус круга в px',
...
ellipse -p [75, 75] -r1 50 -r2 25 -c rgb(0, 255, 0) -b rgba(255, 0, 0, 0.3)`;
    ...
    let canvasModel = new Canvas(ctx, canvas.width, canvas.height);
    ...
    allDrawBtn.addEventListener('click', () => {
        ...
        for (let figure of figures) {
            ...
            // check options
            if (!(/-[A-z]+\d*/g).test(figure)) {
                errorText.innerHTML = errorText.innerHTML + `<p>${figure} неверно заданы параметры</p>`;
                continue;
            }

            // parse options
            let optionNameArray = figure.match(/-[A-z]+\d*/g);
            let optionValueArray = figure.split(/ *-[A-z]+\d* */g);
            let nameFigure = optionValueArray.shift();
            console.log(optionNameArray);
            console.log(optionValueArray);
            console.log(nameFigure);

            // coordinates
            let coords = optionValueArray[optionNameArray.indexOf('-p')];
            if (coords !== undefined) {
                coords = coords.match(/-*\d+/g) || undefined;
            }
            console.log(coords);
            ...
            canvasModel.setStyle(color, bgColor, lineWidth, lineType);
